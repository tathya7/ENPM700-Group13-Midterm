...
HumanDetector::HumanDetector() : frame_width(640), yolo_width(640), frame_height(480), yolo_height(640), nmsthresh(0.45), confidenceThresh(0.45), score_threshold(0.5) {
...
// Processes the image based on a given mode
...
// Draws a bounding box on the detected object
void HumanDetector::drawBbox(int classid, float confidence, int left, int top, int right, int bottom, cv::Mat &frame, const std::vector<std::string> &classes, int uniq_id) {
    cv::rectangle(frame, cv::Point(left, top), cv::Point(right, bottom), cv::Scalar(0, 255, 0), 2);
    std::string label = "Class:" + classes[classid] + "  Confidence:" + cv::format("%.2f", confidence) + "  ID:" + std::to_string(uniq_id);
    int baseLine;
    cv::Size labelSize = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseLine);
    top = std::max(top, labelSize.height);
    cv::rectangle(frame, cv::Point(left, top - labelSize.height), cv::Point(left + labelSize.width, top + baseLine), cv::Scalar(0, 255, 0), cv::FILLED);
    cv::putText(frame, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0), 1);
}

// Removes overlapping boxes based on non-maximum suppression
cv::Mat HumanDetector::rmOverlap(cv::Mat &input_frame, cv::Size &img, std::vector<cv::Mat> &out_imgs, std::vector<std::string> &classes){
    ...
    float x_factor = static_cast<float>(img.width) / static_cast<float>(yolo_width);
    float y_factor = static_cast<float>(img.height) / static_cast<float>(yolo_height);;
    ...
    cv::dnn::NMSBoxes(boxes, class_confidences, score_threshold, nmsthresh, indices);
    ...
    for (int i=0; i<indices.size(); i++){
        ...
        cv::rectangle(boxed_img, cv::Point(left, top), cv::Point(left + width, top + height), cv::Scalar(255, 178, 50), 4);
        ...
        cv::Point b_right = cv::Point(left + label_size.width + 100, top + label_size.height + baseLine);
        // Draw black rectangle for label background
        if (dist2human < 2)
        {
        cv::rectangle(boxed_img, t_left, b_right, cv::Scalar(0, 0, 255), cv::FILLED);
        // Put the label on the rectangle
        cv::putText(boxed_img, dist_label, cv::Point(left + label_size.width + 5, top + label_size.height), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
        }
        else{
        cv::rectangle(boxed_img, t_left, b_right, cv::Scalar(0, 0, 0), cv::FILLED);
        // Put the label on the rectangle
        cv::putText(boxed_img, dist_label, cv::Point(left + label_size.width + 1, top + label_size.height), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 255), 1);
        }
        ...
        // Draw black rectangle for label background
        cv::rectangle(boxed_img, top_left, bottom_right, cv::Scalar(0, 0, 0), cv::FILLED);
        // Put the label on the rectangle
        cv::putText(boxed_img, label, cv::Point(left, top + label_size.height), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 255), 1);
    }
...
void HumanDetector::detect(std::string &input_source){
    ...
    while (1) {
        ...
        cv::dnn::blobFromImage(input_img, blob_img, 1 / 255.0, cv::Size(yolo_width, yolo_height), cv::Scalar(), true, false);
